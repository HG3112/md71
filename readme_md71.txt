...
- dynrng and dynrng2 flags change is not supported. Atmos is not supported.
...
If your eac3-7.1 encoder does not support blu-ray compliant encodes, you can use ffmpeg to create an ac3 core. Standard ffmpeg does not support the encoding into dependent streams, so we can use it only for 5.1 eac3. In this example we use an external eac3-7.1 encoder to get a 4.0 dependent stream, while using ffmpeg for the core. For simplicity, we use the standard ffmpeg 7.1-->5.1 downmix, which perhaps is not optimal for any input. We also set dialn to 31, compr to 00 and chanmpa to 1A00.
